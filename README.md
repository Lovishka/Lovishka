<div align="center">

# Hi, I'm Lovishka Sakuja 👋

**AI & Data Science Undergraduate · Problem Solver**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Lovishka%20Sakuja-0A66C2?style=flat\&logo=linkedin\&logoColor=white)](https://linkedin.com/in/lovishka-sakuja)
[![LeetCode](https://img.shields.io/badge/LeetCode-Rating%201745-FFA116?style=flat\&logo=leetcode\&logoColor=white)](https://leetcode.com)
[![GitHub](https://img.shields.io/badge/GitHub-Follow-181717?style=flat\&logo=github\&logoColor=white)](https://github.com)

</div>

---

## 🚀 About Me

I'm a **B.Tech AI & Data Science student at Galgotias University** (CGPA: **9.38/10**) passionate about building intelligent systems, developer tools, and scalable software solutions. From AI-powered automation platforms to full-stack applications, I enjoy leveraging technology to solve complex problems and create impactful user experiences.

* 🎓 Graduating **July 2027** | Currently in 3rd year
* 🧠 Strong foundation in **OOP**, **DSA**, and **AI workflows**
* 💡 Driven by curiosity, problem-solving, and impactful tech

---

## 🛠️ Tech Stack

### Frontend

![HTML](https://img.shields.io/badge/HTML5-E34F26?style=flat\&logo=html5\&logoColor=white)
![CSS](https://img.shields.io/badge/CSS3-1572B6?style=flat\&logo=css3\&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat\&logo=javascript\&logoColor=black)
![React](https://img.shields.io/badge/React-61DAFB?style=flat\&logo=react\&logoColor=black)

### Languages & Databases

![Java](https://img.shields.io/badge/Java-ED8B00?style=flat\&logo=openjdk\&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat\&logo=python\&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat\&logo=mysql\&logoColor=white)

### Libraries & Tools

![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat\&logo=pandas\&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat\&logo=numpy\&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=flat)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat\&logo=fastapi\&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=flat\&logo=opencv\&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat\&logo=git\&logoColor=white)

---

## 🌟 Featured Projects

### 🚀 [RefactorIQ – Intelligent Code Modernization Engine](https://github.com/Lovishka/RefactorIQ)

> *AI-powered software intelligence · React · FastAPI · Tree-Sitter · LLMs*

Enterprise-grade AI platform that analyzes software architecture, detects technical debt, visualizes dependency graphs, and generates autonomous refactoring recommendations to improve code quality and developer productivity.

* 📡 Repository-wide dependency visualization
* 🤖 AI-generated refactoring suggestions
* 🛡️ Dead code & circular dependency detection
* 📊 Technical debt analytics and code health monitoring
* 💰 Engineering ROI tracker for development time savings

---

### 🏥 [HealTrack AI – Smart Hospital Discharge Automation](https://github.com/Lovishka/HealTrackAI)

> *Full-stack workflow automation · Java · FastAPI · React · SQL*

Automated hospital patient discharge system that replaces manual paperwork and long queues with AI-driven digital coordination — significantly reducing discharge time and administrative workload.

---

## 🏆 Achievements

* 💻 **600+ DSA problems** solved across LeetCode, Codeforces & GeeksforGeeks
* 🧠 **LeetCode Rating: 1699**
* 🎓 **CGPA: 9.38/10**
* 🚀 Built AI-powered and full-stack software solutions

---

## 📫 Let's Connect!

I'm always open to collaborating on interesting projects, discussing tech, or just saying hi!

* 💼 **LinkedIn**: [Lovishka Sakuja](https://linkedin.com/in/lovishka-sakuja)

---

<div align="center">

*"Driven by curiosity, powered by code."* ✨

</div>
